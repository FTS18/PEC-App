  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold font-monument uppercase tracking-tight">Faculty Nexus</h1>
        ...
        {[
          ...
          <div key={label} className="card-elevated p-6 border-b-4 border-r-4 border-primary/20">
            <div className="flex items-center gap-4">
              ...
              <div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">{label}</p><p className="text-3xl font-black font-monument mt-1">{value}</p></div>
            </div>
          </div>
        ))}
      </div>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-xl rounded-sm border-2 border-primary shadow-[15px_15px_0px_rgba(0,0,0,0.1)]">
          <DialogHeader className="space-y-4">
            <DialogTitle className="text-3xl font-black font-monument uppercase tracking-tight">{editingFaculty ? 'Modify Persona' : 'Initialize Identity'}</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={showBulkUpload} onOpenChange={setShowBulkUpload}>
        <DialogContent className="max-w-4xl border-2 border-primary rounded-sm overflow-hidden p-0">
          <DialogHeader className="bg-primary text-white p-10">
            <DialogTitle className="text-3xl font-black font-monument uppercase">Mass Enrollment Protocol</DialogTitle>
          </DialogHeader>
          <div className="p-10">
            <BulkUpload
                entityType="faculty"
