...
export function StudentProfileView({ userData, profileData, stats, githubStats, currentUser }: any) {
  ...
  const handleMessage = () => {
    ...
    router.push(`/chat?userId=${userData.uid}`);
  };
            ...
            {profileData.githubUsername && (
              <div className="card-elevated p-4">
                ...
                {githubStats ? (
                  ...
                  <div className="text-xs text-muted-foreground">GitHub info not available</div>
                )}
              </div>
            )}
          </motion.div>
